...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Killuminati " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"killuminati.info.tm", "killuminatidnsseed.info.tm"},
	{"killuminati.info.tm", "killuminatidnsseed.info.tm"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Killuminati is probably already running."), addrBind.ToString().c_str());
        else
